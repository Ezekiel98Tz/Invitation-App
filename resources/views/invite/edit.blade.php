@php
    ...
    $answers = (array) ($guest->rsvp?->answers ?? []);
@endphp
...
<x-guest-layout>
    <div
        x-data="inviteRsvp({
            ...
            initial: {
                status: @js($guest->status),
                attending_count: {{ (int) ($guest->rsvp?->attending_count ?? 1) }},
                answers: @js($answers),
            },
        })"
        class="space-y-6"
    >
        ...
        <div x-show="!confirmed" x-cloak class="rounded-md bg-blue-50 p-3 text-sm text-blue-700">
            You already responded. You can update your RSVP until the event starts.
        </div>

        <div x-show="confirmed" x-cloak class="space-y-4 text-center">
            <div class="rounded-md bg-green-50 p-3 text-sm text-green-700">
                <span x-show="confirmedData.status === 'accepted'">Your RSVP is updated: attending.</span>
                <span x-show="confirmedData.status === 'declined'">Your RSVP is updated: not attending.</span>
            </div>
        ...
        <form x-show="!confirmed" x-cloak class="space-y-4" method="POST" action="{{ route('invites.store', ['token' => $guest->invite_token], absolute: false) }}" x-on:submit.prevent="submit">
            ...
            <div class="space-y-2">
                <button
                    ...
                    :disabled="loading"
                    x-text="loading ? 'Saving...' : 'Update RSVP'"
                ></button>
